tag @s add Honey_B_Player
#execute anchored eyes run summon snowball ^ ^ ^1 {Tags:["Honey_B_Throw"],Passengers:[{id:"minecraft:marker",Tags:["Honey_B_marker"]}]}
#execute as @e[tag=Honey_B_Throw,limit=1,sort=nearest] at @s run function crp_item:thief_i/honey_bottle/motion
#実行者 -> コマンドを実行するエンティティ
#実行座標 -> コマンドを実行するエンティティ


#矢の召喚
execute anchored eyes run summon snowball ^ ^ ^1 {Tags:["Honey_B_Marking","Honey_B_Throw"],Passengers:[{id:"minecraft:marker",Tags:["Honey_B_marker"]}],Item:{id:"minecraft:snowball",count:1,components:{"minecraft:item_model":"minecraft:air"}}}
